            ...
            <div className="mx-auto max-w-5xl px-6 py-16 sm:py-20">
                ...
                <div className="mt-12 grid gap-8 md:grid-cols-2">
                    {SPECIALTIES.map((group) => (
                        <div key={group.title} className="rounded-2xl border border-header-primary/10 bg-white/70 p-5 shadow-[0_20px_40px_rgba(24,25,35,0.08)] sm:p-6">
                            <h3 className="font-cinzel text-xl text-header-primary sm:text-2xl">
                                {group.title}
                            </h3>
                            {group.description && (
                                <p className="mt-2 text-xs font-light text-foreground/60 sm:text-sm">
                                    {group.description}
                                </p>
                            )}
